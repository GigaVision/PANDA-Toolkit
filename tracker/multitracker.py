...
class JDE_PANDA_Tracker(object):
    ...
    def update(self, sub_img_list):
        ...
        for img_tuple in sub_img_list:
            img0, img, loc_info = img_tuple
            im_blob = torch.from_numpy(img).cuda().unsqueeze(0)
            with torch.no_grad():
                pred_sub = self.model(im_blob)

            # filter objects whose confidence is low & very close to edge (mostly cut by edge of windows)
            judge = (pred_sub[:, :, 4] > self.opt.conf_thres) & \
                    ((pred_sub[:, :, 0] - pred_sub[:, :, 2] / 2) > self.edge_margin) & \
                    ((pred_sub[:, :, 1] - pred_sub[:, :, 3] / 2) > self.edge_margin) & \
                    ((pred_sub[:, :, 0] + pred_sub[:, :, 2] / 2) < (self.width - self.edge_margin)) & \
                    ((pred_sub[:, :, 1] + pred_sub[:, :, 3] / 2) < (self.height - self.edge_margin))
            pred_sub = pred_sub[judge]
            if len(pred_sub) > 0:
                disp = torch.zeros(pred_sub.size(), device='cuda:0')
                disp[:, 0], disp[:, 1] = loc_info[1:] 

                pred_sub = torch.add(pred_sub, disp)
