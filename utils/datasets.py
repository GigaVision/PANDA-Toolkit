...
class LoadPandaImages:  # for inference
    def __init__(self, path, img_size=(1088, 608), overlap=0.2, scales=(0.2,0.4)):
        if os.path.isdir(path):
            image_format = ['.jpg', '.jpeg', '.png', '.tif']
            self.files = sorted(glob.glob('%s/*.*' % path))
            self.files = list(filter(lambda x: os.path.splitext(x)[1].lower() in image_format, self.files))
    ...
    def __next__(self):
        ...
        toc = time.time()
        print('1    JPEG decoding cost time: {} s'.format(toc-tic))
        ...
        toc = time.time()
        print('2    sliding window cutting cost time: {} s'.format(toc-tic))  # '  window number: ', len(sub_img_list)
